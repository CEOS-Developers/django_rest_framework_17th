...
가비아 도메인을 구매한 기준 순서는 다음과 같다.

1. 가비아에서 도메인을 구매한다.(숙제용 이므로 store 나 shop 으로 끝나는 도메인을 구매하는 것을 추천한다.)
   * [링크](https://developer-ping9.tistory.com/320) 링크대로 등록을 해준다. 
2. ACM에서 인증서를 발급 받는다. 이 때!!!! 반드시 ACM에서 도메인 항목에 있는 **Route 53 에서 레코드 생성** 이라는 버튼을 클릭할 수 있도록 한다. (안 그러면 허가가 나지 않는다.)
3. 타겟 그룹과 로드밸런서 생성. 이 때 443 포트를 생성할 때 protocol을 HTTPS로 바꿔줘야 인증서를 등록할 수 있다.
4. route53 도메인에 레코드에 ALB를 등록해준다.
...
* DJANGO_ALLOWED_HOST를 제대로 설정하지 않아서 400 에러가 나왔다.... 이거 때문에 숙제의 많은 시간을 날렸다.
* 아마존의 UX는 그저... 전설이다.... 숙제하면서 이거 때문에 몇번 애를 먹었는지 모르겠다.
* 아마존은 새로운 서비스를 만드는게 아니라 UX부터 개선하면 좋겠다. 